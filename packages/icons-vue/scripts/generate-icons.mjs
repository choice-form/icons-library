 ...
 * Vue 图标生成器类
 */
class VueIconGenerator extends BaseIconGenerator {
  constructor() {
    super({
      packageRoot,
      iconsDir,
   ...
   * 创建Vue组件内容
   */
  createVueComponent(componentName, svgContent) {
...
export default {
  ...
  async generateComponentFile(componentName, optimizedSvg, outputPath) {
    const componentCode = this.createVueComponent(componentName, optimizedSvg);
  ...
  async generateIndexFile(components) {
    ...
    const indexContent =
      components
        .map(
          (comp) =>
            `export { default as ${comp.name} } from './icons/${comp.name}.vue';`
        )
...
async function run() {
  try {
    const generator = new VueIconGenerator();
